...
abstract contract D_Swap_Factory  {
    ...
    function Create(address user, address token_head,address token_tail,uint256 sys_reward)virtual payable public  returns(address);
...
contract D_Swap_Mains is Owned {
    ...
    function Create(address token_head,address token_tail,uint256 sys_reward) payable public  returns(address){
        ...
        if(sys_reward>0)
        {
            ...
            sys_res=ERC20Interface(m_Address_of_System_Reward_Token).transferFrom(msg.sender, address(this),sys_reward);
            if(sys_res ==false)
            {
        ...
        address res= address(D_Swap_Factory(m_Factory_Lib).Create( msg.sender,token_head,token_tail,sys_reward));
        m_My_Dear_Son[res]=true;
        Triger_Create(res , msg.sender, msg.sender , token_head, token_tail,sys_reward);

        if(sys_reward>0)
        {
            sys_res=ERC20Interface(m_Address_of_System_Reward_Token).transfer(res,sys_reward/3*2);
            if(sys_res ==false)
            {
                //if failed revert transaction;
                 revert();
    ...
    function Triger_Create(address swap ,address user,address swap_owner ,address token_head,address token_tail ,uint256 sys_reward)private
    {
            emit E_Create( swap , user, swap_owner , token_head, token_tail,sys_reward);
    }
    
    function Triger_Entanglement(address swap ,address user,address op_token_head,address op_token_tail)public My_Dear_Son
    {
            emit E_Entanglement( swap , user,op_token_head,op_token_tail);
    }
    function Triger_Initialize(address swap ,address user,uint256 total_amount_head ,uint256 total_amount_tail ,uint256 limit_head ,uint256 limit_tail ,address rival_head,address rival_tail , uint256 pair_dlo ,uint256 option_dlo)public My_Dear_Son
    {
            emit E_Initialize( swap , user, total_amount_head , total_amount_tail , limit_head , limit_tail , rival_head, rival_tail ,  pair_dlo , option_dlo);
    }
    function Triger_Claim_For_Head (address swap ,address user,address referer)public My_Dear_Son
    {
            emit  E_Claim_For_Head( swap , user ,referer);
    }
    function Triger_Claim_For_Tail(address swap ,address user,address referer)public My_Dear_Son
    {
         emit E_Claim_For_Tail( swap , user ,referer);
    }
    function Triger_Deposit_For_Head(address swap ,address user, uint256 amount , uint256 deposited_amount)public My_Dear_Son
    {
        emit E_Deposit_For_Head( swap , user,  amount ,deposited_amount);
    }
    function Triger_Deposit_For_Tail(address swap ,address user, uint256 amount, uint256 deposited_amount)public My_Dear_Son
    {
        emit E_Deposit_For_Tail( swap , user,  amount,deposited_amount);
    }
